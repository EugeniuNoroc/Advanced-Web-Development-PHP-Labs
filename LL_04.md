...
Создан новый PHP-файл `index.php`, в котором подключена строгая типизация:
...
Создан массив `$transactions`, содержащий информацию о банковских транзакциях. Каждая транзакция представлена ассоциативным массивом с полями:
...
С помощью `foreach` реализован вывод списка транзакций в HTML-таблице:
...
Создана директория `image/` с размещёнными в ней изображениями формата `.jpg`. Реализован файл `index.php` со следующей структурой:
